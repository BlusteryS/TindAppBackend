...
public class TranslationService {
    ...
    public Future<Optional<Message.MessageTranslation>> translate(final String text, final String sourceLanguage, final String targetLanguage) {
        ...
        return Future.fromCompletionStage(httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString()))
            .map(response -> {
                if (response.statusCode() / 100 != 2) {
                    ...
                    return Optional.<Message.MessageTranslation>empty();
                }
                ...
                return Optional.of(new Message.MessageTranslation(normalizedTarget, normalizedSource, translatedText));
            })
            .otherwise(error -> {
                logger.warn("Failed to translate text", error);
